...
public class TC_DemoQA_059 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Sheet s = wb.getSheet("TC_DemoQA_059");
			...
			List<WebElement> tabs = driver.findElements(By.xpath(s.getCell(4, 0).getContents()));
			Thread.sleep(2000);

			for (int i = 0; i <= tabs.size(); i++) {
				if (tabs.get(i).getText().contains(s.getCell(5, 0).getContents()))
